...
int renderCow(int bytesPerPixel, u32 width, u32 height)
{
    ...
    const u32 ntris = 3156;
    const f32 nearClippingPLane = 1;
    const f32 farClippingPLane = 1000;
    f32 focalLength = 20; // in mm

    // 35mm Full Aperture in inches
    f32 filmApertureWidth = 0.980f;
    f32 filmApertureHeight = 0.735f;
	...
	for (uint32_t i = 0; i < ntris; ++i) {
		...
		uint32_t x0 = std::max(i32(0), (int32_t)(std::floor(xmin)));
		uint32_t x1 = std::min(i32(imageWidth) - 1, (int32_t)(std::floor(xmax)));
		uint32_t y0 = std::max(i32(0), (int32_t)(std::floor(ymin)));
		uint32_t y1 = std::min(i32(imageHeight) - 1, (int32_t)(std::floor(ymax)));
